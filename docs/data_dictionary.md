...
The Bronze layer preserves source data as ingested. The Silver layer applies validation, standardization, deduplication, and approved corrections. The Gold layer contains analytics-ready datasets.

Historical observations analyzed so far cover:
...
The logical grain of a historical FIPE observation is:
...
| `nome_marca` | `str` | No | Vehicle brand name published by the source. | Normally stable for a `codigo_fipe`, but historical spelling/canonicalization inconsistencies exist. |
...
| `ano_referencia` | `int32` | No | FIPE reference year. | Historical dataset analyzed from `2001` through `2026`; future valid years are expected as the pipeline advances. |
...
The pipeline must validate the 1:1 relationship between `sigla_combustivel` and `nome_combustivel`.
...
All other structural columns are non-null in the historical dataset.

---

## 6. Historical Cardinality Notes
...
Historical coverage is continuous from:
...
Number of monthly reference periods:
...
## 10. Bronze / Silver Interpretation

### Bronze

Bronze preserves the source exactly as received.

No rows are deleted or corrected in Bronze.
...
Silver is expected to:

- enforce schema and structural rules
- remove exact duplicate rows
- standardize data types
- validate the logical grain
- apply only high-confidence canonicalization
- quarantine ambiguous or invalid records
- preserve audit metadata for corrections
...
A derived monthly reference date may also be created in Silver:
...
This data dictionary describes the structure and observed behavior of the dataset, while `data_quality_rules.md` defines the validation and treatment logic applied by the pipeline.
